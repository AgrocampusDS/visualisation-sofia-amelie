...
plot_ly(dta_SYR) %>%
  ...
  add_annotations(x = 14.5, y = 69.6, 
    text  = "Guerre civile depuis 2011", 
    xanchor = 'center',
    showarrow = F)  %>%
  add_annotations(x = 9, y = 69.2, 
    text = "Chef du gouvernement : Bashar el-Assad", 
    xanchor = 'center',
    showarrow = F)  %>%
  add_annotations(x = 6.2, y = 74.6, 
    text = "Retrait des troupes syriennes du Liban", 
    xanchor = 'left',
    showarrow = F)
...
ggplot(data = dta_SYR) +
  ...
  geom_line(aes(x = Year, y = Depense), 
            color = "grey", size = 1, group=1) +
  
  geom_point(aes(x = Year, y = Depense, group=1)) +
  
  scale_y_continuous(breaks = seq(3, 5.5, 0.5)) +
